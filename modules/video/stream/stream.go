...
func (s *stream) HandleVideoCreated(ctx context.Context, req *pb.HandleVideoCreatedRequest) (*emptypb.Empty, error) {
	...
	variant := []int32{1080, 720, 480, 320}
	for _, scale := range variant {
		videoCreateReq := &pb.HandleVideoCreatedRequest{
			Id:    	req.GetId(),
			Url:    req.GetUrl(),
			Scale:  scale,
		}
		if err := s.produceVideoCreatedWithScaleEvent(videoCreateReq); err != nil {
			return nil, &saramakit.HandlerError{Retry: true, Err: err}
		}
	}
